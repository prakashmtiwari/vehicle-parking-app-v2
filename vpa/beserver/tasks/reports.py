...
def get_user_Reservationss_for_month(user_id: int, year: int, month: int):
    """
    Return Reservationss for the user in given month.
...
def get_user_monthly_stats(user_id: int, year: int, month: int):
    ...
    # you can add other stats: average duration, peak hour etc.
    return {
        "total_Reservationss": total_Reservationss,
...
<body>
  ...
  <div class="card">
    <h2>Summary</h2>
    <p><strong>Total Reservationss:</strong> {{ stats.total_Reservationss }}</p>
    <p><strong>Total amount spent:</strong> ₹{{ "%.2f"|format(stats.total_amount) }}</p>
    {% if stats.most_used_lot %}
      <p><strong>Most used parking lot:</strong> {{ stats.most_used_lot.name }} ({{ stats.most_used_lot.count }} Reservationss)</p>
    {% else %}
  ...
  <div class="card">
    ...
    {% if Reservationss %}
      <table>
        ...
        <tbody>
        {% for b in Reservationss %}
          <tr>
            <td>{{ b.parking_timestamp.strftime("%Y-%m-%d %H:%M") }}</td>
            <td>{{ b.lot_name }}</td>
            <td>{{ b.duration }}</td>
    ...
    {% else %}
      <p>No Reservationss during this month.</p>
    {% endif %}
  </div>
...
# --- uses your existing send_gmail_message (HTTP API) if available ---
import os
import requests
import logging
...
# --- Report builder ---
def build_report_html(user, Reservationss, stats, year, month):
    # enrich Reservationss for template
    enriched = []
    for b in Reservationss:
        lot = db.session.query(Parking_Lot).filter(Parking_Lot.id == b.lot_id).first()
        duration = ""
        if getattr(b, "start_time", None) and getattr(b, "end_time", None):
            delta = b.end_time - b.start_time
            duration = str(delta)
            logger.info(f"Computed duration for reservation {b.id}: {duration}")
        enriched.append({
            "created_at": b.parking_timestamp,
...
def send_monthly_activity_report(self):
    ...
    for user in users:
        ...
        Reservationss = get_user_Reservationss_for_month(user.id, report_year, report_month)
        stats = get_user_monthly_stats(user.id, report_year, report_month)

        print(f"User {user.username} - Reservationss: {len(Reservationss)} - Stats: {stats}")
        if Reservationss and stats:
            try:
                html = build_report_html(user, Reservationss, stats, report_year, report_month)
            ...
            logger.info("No Reservationss or stats for user %s, skipping report", user.username)
